...
-- 2) Tighten RLS on meeting_choices: restrict SELECT to same group
DROP POLICY IF EXISTS "Users can view all choices in accessible polls" ON public.meeting_choices;
CREATE POLICY "Users can view choices from their group"
...
-- 4) Optional: provide a helper to count distinct users in the same group (can be used by frontend later)
CREATE OR REPLACE FUNCTION public.get_group_user_count(poll_id_param uuid)
RETURNS integer
LANGUAGE sql
STABLE
SECURITY DEFINER
